...
files.forEach(fPath => {
    const fullPath = path.join(basePath, fPath);
    ...
    if (match) {
        ...
        if (splitStart.length >= 2) {
            ...
            if (nextFuncIndex !== -1) {
                ...
                console.log(`Couldn't find natural ending for ${fName}. Attempting fallback replacement...`);
                // Fallback: Just try to replace the whole remaining block up to `saveHistory()` if it exists
                // The regex split strategy failed cleanly, so let's check manually if we can slice it
            }
        }
    } else {
...
fs.writeFileSync('c:/Users/NATHAN/OneDrive/ANTIGOS/Documentos/GitHub/APsycho/phase6_gabs.txt', collectedGabs.join('\n'));
