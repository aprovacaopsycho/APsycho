...
# Script para corrigir o padrão de gabarito mobile em todos os testes de inteligência
# Substitui o painel com x-show aninhado por botões DENTRO do card da questão

$basePath = "c:\Users\NATHAN\OneDrive\ANTIGOS\Documentos\GitHub\APsycho\Base"

# Padrão a procurar e substituir em cada arquivo
# A lógica: dentro do card da questão, antes do div de navegação (Anterior/Próxima),
# inserir os botões de resposta mobile (lg:hidden)
...
foreach ($cfg in $filesConfig) {
    ...
    # Verificar se já tem botões mobile dentro do card
    if ($content -match "BOTÕES DE RESPOSTA MOBILE") {
        Write-Host "SKIP: $($cfg.file) - já tem botões mobile." -ForegroundColor Yellow
        continue
    }
    
    # Primeiro remover o painel GABARITO MOBILE separado se existir (mal implementado)
    # Identificar o padrão do painel problemático
    $oldPanel = @"

                    <!-- GABARITO MOBILE (visivel apenas em mobile/tablet) -->
"@
    
    # Construir o bloco de botões mobile a inserir
    $mobileButtons = @"
...
"@
    ...
    # Encontrar o ponto de inserção: antes do comentário "Botão Próximo"
    $anchor = "                        <!-- Botão Próximo"
    
    # Verificar se o anchor existe
    if ($content -notmatch [regex]::Escape($anchor)) {
        # Tentar alternativa
        $anchor = "                        <!-- Botão Navigation"
    }
    ...
    # Inserir os botões ANTES do botão próximo/anterior
    $newContent = $content.Replace($anchor, $mobileButtons + $anchor)
    
    # Também remover o bloco GABARITO MOBILE separado se existir
    # Procurar padrão do bloco problemático e removê-lo
    $pattern = '(?s)\s*<!-- GABARITO MOBILE \(visivel apenas em mobile/tablet\) -->.*?</div>\s*\n\s*</div>\s*\n\s*<!-- fim AREA DO TESTE -->'
    $replacement = "`n                </div>`n                <!-- fim AREA DO TESTE -->"
    $newContent = [regex]::Replace($newContent, $pattern, $replacement)
    
    if ($newContent -eq $content) {
        Write-Host "AVISO: Nenhuma mudança aplicada em $($cfg.file)" -ForegroundColor Cyan
    } else {
