...
"@
    ...
    $anchor1 = "                        <!-- Botão Próximo"
    $anchor2 = "                        <!-- Botão Navigation"
    
    $anchor = $null
    if ($content -match [regex]::Escape($anchor1)) {
        $anchor = $anchor1
    } elseif ($content -match [regex]::Escape($anchor2)) {
        $anchor = $anchor2
    }
    ...
    # Remover o bloco GABARITO MOBILE antigo (pode haver variações em como foi injetado)
    $pattern1 = '(?s)\s*<!-- GABARITO MOBILE \(visivel apenas em mobile/tablet\) -->.*?</div>\s*\n\s*</div>\s*\n\s*<!-- fim AREA DO TESTE -->'
    $pattern2 = '(?s)\s*<!-- GABARITO MOBILE \(visivel apenas em mobile/tablet\) -->.*?</details>\s*\n\s*</div>'
